...
theta_1_reg = sum(sum(Theta1(:,2:end) .^2));
theta_2_reg = sum(sum(Theta2(:,2:end) .^2));


J = (sum(sum(-y_matrix' .* log(a_3) .- ((1 .- y_matrix)' .* log(1 .- a_3)))))*(1/m) + (lambda/(2*m))*(theta_1_reg+theta_2_reg);
